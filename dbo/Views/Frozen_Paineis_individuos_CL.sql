CREATE VIEW [dbo].[Frozen_Paineis_individuos_CL]  
AS  
select DISTINCT  
...
FROM  [FullPanellistStateChanges] WHERE CountryISO2A='CL' AND ToState='PanelistDroppedOffState'  AND [ReasonCode] IS NOT NULL   
) fpl on fpl.CountryISO2A='CL' and fpl.PanelCode=p.PanelCode and   
...
--where ac.CountryISO2A='CL' and a.[Key]='Dateofresidency'  
...
inner join Country ac on ac.CountryId=av.Country_Id  
left outer join Attribute a on a.GUIDReference=av.DemographicId   
where ac.CountryISO2A='CL' and a.[Key]='CensoYear'  
...
--EXEC sys.sp_addextendedproperty @name=N'Description', @value=N'Gives Panellist details like panel code ,signup date, drop out date , reason ' , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'VIEW',@level1name=N'Frozen_Paineis_individuos_CL'
--GO
