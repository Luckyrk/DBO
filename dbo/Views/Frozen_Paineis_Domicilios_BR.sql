GO  
CREATE VIEW [dbo].[Frozen_Paineis_Domicilios_BR]  
AS   
select DISTINCT  
...
inner join collective cl on cl.GUIDReference=pl.PanelMember_Id  
...
left outer join (SELECT ROW_NUMBER() OVER(PArtition BY PanelMemberId,PanelCode ORDER BY [Date] DESC) AS RNO,* FROM [FullPanellistStateChanges] WHERE CountryISO2A='BR' AND ToState='PanelistDroppedOffState' ) fpl   
on fpl.CountryISO2A='BR' and fpl.PanelCode=p.PanelCode and fpl.PanelMemberID=cast(cl.Sequence as nvarchar(10)) and  fpl.ToState='PanelistDroppedOffState'   
AND RNO=1  
where c.CountryISO2A='BR'  

GO
EXEC sys.sp_addextendedproperty @name=N'Description', @value=N'Gives Panellist details like panel code , drop out date , reason along with these columns date of residency of the members of that group ' , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'VIEW',@level1name=N'Frozen_Paineis_Domicilios_BR'
GO

--GRANT SELECT ON [Frozen_Paineis_Domicilios_BR] TO GPSBusiness
