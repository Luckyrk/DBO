GO  
CREATE VIEW [dbo].[Frozen_Paineis_Domicilios_MX]  
AS   
select DISTINCT  
...
inner join collective MX on MX.GUIDReference=pl.PanelMember_Id  
...
left outer join (SELECT ROW_NUMBER() OVER(PArtition BY PanelMemberId,PanelCode ORDER BY [Date] DESC) AS RNO,* FROM [FullPanellistStateChanges] WHERE CountryISO2A='MX' AND ToState='PanelistDroppedOffState' ) fpl   
on fpl.CountryISO2A='MX' and fpl.PanelCode=p.PanelCode and fpl.PanelMemberID=cast(MX.Sequence as nvarchar(10)) and  fpl.ToState='PanelistDroppedOffState'   
AND RNO=1  
where c.CountryISO2A='MX'  

GO

--EXEC sys.sp_addextendedproperty @name=N'Description', @value=N'Gives Panellist details like panel code , drop out date , reason along with these columns date of residency of the members of that group ' , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'VIEW',@level1name=N'Frozen_Paineis_Domicilios_MX'
--GO

--GRANT SELECT ON [Frozen_Paineis_Domicilios_MX] TO GPSBusiness
