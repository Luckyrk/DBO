...
CREATE VIEW [dbo].[IncentiveAccountBalance_ByHouseholdPanel]
AS
...
INNER JOIN dbo.CollectiveMembership cm ON ind.GUIDReference = cm.Individual_Id
INNER JOIN dbo.Collective cl ON cl.GUIDReference = cm.Group_Id
INNER JOIN dbo.Candidate c ON c.GUIDReference = cm.Individual_ID --GUIDReference
INNER JOIN dbo.Country AS cnt ON ind.CountryId = cnt.CountryId
INNER JOIN dbo.Panelist p ON cl.GUIDReference = p.PanelMember_ID
