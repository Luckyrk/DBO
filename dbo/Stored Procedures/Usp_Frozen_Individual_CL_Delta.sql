CREATE PROCEDURE [dbo].[Usp_Frozen_Individual_CL_Delta]  
(  
...
BEGIN  
 ...
 DECLARE @ViewName VARCHAR(500)='Frozen_Individual_CL',@CountryFromDate DATETIME ,@CountryCurrentDate DATETIME ,@ServerFromDate DATETIME
 ...
 IF Object_Id('dbo.Frozen_Individual_CL_Delta_Current') IS NOT NULL  
 BEGIN  
 DROP TABLE Frozen_Individual_CL_Delta_Current  
 END  
 ...
 END 
 CREATE TABLE [dbo].[Frozen_Individual_CL_Delta_Current](  
  idDomicilio int,  
...
AND A.[Key] IN ('I601','I604','I607','I605','I609','I610','I602','I606','I603','I612','I613','I608')  
...
FROM [Frozen_Individual_CL_DeletedRecords]  
...
INSERT INTO Frozen_Individual_CL_Delta_Current (idDomicilio,idIndividuo,data_Inicial,nome_individuo,Sexo,Data_Nascimento,idInstrucao,
...
Iduso_internet,Idlocal_internet,TipoSalud,Load_date,[Country_Load_date])        
 ...
 ,T1.idTipoEscuela,T1.Iduso_internet,T1.Idlocal_internet,T1.TipoSalud,@ServerCurrentDate,@CountryCurrentDate         
 FROM Frozen_Individual_CL T1        
 ...
 LEFT JOIN Frozen_Individual_CL_Delta_Current T2 ON T1.idDomicilio=T2.idDomicilio AND T1.idIndividuo=T2.idIndividuo       
 WHERE T2.idDomicilio IS NULL        
       
 DELETE FROM B          
 FROM FrozenIndividualChanges D          
 JOIN Frozen_Individual_CL_Delta_BackUp B ON B.idDomicilio=D.idDomicilio AND B.idIndividuo=D.idIndividuo           
 WHERE D.isDeleted=1        
       
  
 MERGE INTO Frozen_Individual_CL_Delta_Current AS tgt  
USING  Frozen_Individual_CL_Delta_BackUp AS src  
...
INSERT INTO Frozen_Views_Delta_Latam  
 ...
 FROM Frozen_Individual_CL_Delta_Current T1   
 WHERE isUpdated=1  
   UNION        
 ...
 DELETE B FROM   
 Frozen_Individual_CL_Delta_BackUp B  
 JOIN Frozen_Individual_CL_Delta_Current C ON C.idDomicilio =B.idDomicilio AND C.idIndividuo=B.idIndividuo  
 WHERE C.isUpdated=1  
    
 INSERT INTO Frozen_Individual_CL_Delta_BackUp  
 SELECT * FROM Frozen_Individual_CL_Delta_Current  
 WHERE isUpdated=1  
