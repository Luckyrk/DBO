CREATE PROCEDURE [dbo].[Usp_Frozen_Paineis_individuos_BR_Delta]
(    
...
BEGIN    
 ...
 DECLARE @ViewName VARCHAR(500)='Frozen_Paineis_individuos_BR',@CountryFromDate DATETIME ,@CountryCurrentDate DATETIME ,@ServerFromDate DATETIME    
 ...
 IF Object_Id('dbo.Frozen_Paineis_individuos_BR_Delta_Current') IS NOT NULL    
 BEGIN    
 DROP TABLE Frozen_Paineis_individuos_BR_Delta_Current    
 END    
 IF Object_Id('dbo.FrozenPaineisindividuosChanges') IS NOT NULL    
 BEGIN    
 DROP TABLE FrozenPaineisindividuosChanges    
 END
 CREATE TABLE [dbo].[Frozen_Paineis_individuos_BR_Delta_Current](    
   idpainel INT,    
   iddomicilio INT,    
   idindividuo BIGINT,    
...
FROM Frozen_Paineis_individuos_BR WHERE ((PLGPSUpdateTimeStamp>= @CountryFromDate AND PLGPSUpdateTimeStamp <= @CountryCurrentDate) OR    
  (CMGPSUpdateTimeStamp>= @CountryFromDate AND CMGPSUpdateTimeStamp <= @CountryCurrentDate))        
UNION       
SELECT idpainel,iddomicilio,idindividuo,0 AS isDeleted      
FROM [Frozen_Paineis_individuos_BR_DeletedRecords]  WHERE  ((PLGPSUpdateTimeStamp>= @CountryFromDate AND PLGPSUpdateTimeStamp <= @CountryCurrentDate))    
    
 INSERT INTO Frozen_Paineis_individuos_BR_Delta_Current (idpainel,iddomicilio,idindividuo,Data_Entrada,    
 ...
 FROM Frozen_Paineis_individuos_BR  T1    
 ...
 LEFT JOIN Frozen_Paineis_individuos_BR_Delta_Current T2 ON T1.idpainel=T2.idpainel AND T1.iddomicilio=T2.iddomicilio AND T1.idindividuo=T2.idindividuo     
 WHERE T2.idpainel IS NULL    
   
 DELETE FROM B      
 FROM FrozenPaineisindividuosChanges D      
 JOIN Frozen_Paineis_individuos_BR_Delta_BackUp B ON B.iddomicilio=D.iddomicilio AND B.idpainel = D.idpainel AND B.idindividuo=D.idindividuo  
 WHERE D.isDeleted=1      
     
    
 MERGE INTO Frozen_Paineis_individuos_BR_Delta_Current AS tgt    
USING  Frozen_Paineis_individuos_BR_Delta_BackUp AS src    
...
INSERT INTO Frozen_Views_Delta_Latam    
 ...
 FROM Frozen_Paineis_individuos_BR_Delta_Current T1     
 WHERE isUpdated=1     
  UNION    
  SELECT iddomicilio,@ServerCurrentDate,NULL,@CountryCode,@ViewName       
...
DELETE B FROM       
Frozen_Paineis_individuos_BR_Delta_BackUp B      
JOIN Frozen_Paineis_individuos_BR_Delta_Current C ON C.idpainel=B.idpainel AND C.iddomicilio=B.iddomicilio AND C.idindividuo=B.idindividuo     
WHERE C.isUpdated=1      
        
  INSERT INTO Frozen_Paineis_individuos_BR_Delta_BackUp      
  SELECT * FROM Frozen_Paineis_individuos_BR_Delta_Current      
  WHERE isUpdated=1      
