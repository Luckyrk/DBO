CREATE Procedure [dbo].[Audit_CreateAuditTrigger_Insert]
...
set @dropStmt ='IF OBJECT_ID('+@quote+'trg'+@tablename+'_I'+@quote+') IS NOT NULL 
       DROP TRIGGER dbo.trg'+@tablename+'_I' 

--this is just the header info  for the trigger 
SET @sqlHeader ='CREATE TRIGGER dbo.trg'+@tablename+'_I
ON dbo.['+@tablename+'] FOR insert 
AS 
' 
...
set @allsqlStatement=@allsqlStatement+@sqlColumns + ',''I'' ' --add the operation of the trigger
 
set @sqlJoin = 'from inserted i' 

set @allsqlStatement=@allsqlStatement+@sqlJoin

Declare @Enabled bit
Select @Enabled=TriggerEnabledInsert from  AuditConfig Where TableName=@tablename
if @Enabled=1
Begin
	 exec (@dropStmt)
...
End 
Else	
	Print 'The Flag "TriggerEnabledInsert" allocated in dbo.AuditConfig for the table ['+@tablename +'] is set to 0. No Action performed' 	
END TRY
BEGIN CATCH
