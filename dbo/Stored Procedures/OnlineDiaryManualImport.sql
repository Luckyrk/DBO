CREATE PROCEDURE [dbo].[OnlineDiaryManualImport] (
	...
	,@SingleFileName NVARCHAR(500)
	,@AuditID NVARCHAR(100)
	,@FileDate NVARCHAR(100)
	)
AS
/*##########################################################################
-- Name				: OnlineDiaryManualImport
-- Date             : 2015-01-06
-- Author           : Jagadeesh B
-- Company          : Cognizant Technology Solution
-- Purpose          : Online Dairy manula import
-- Usage			:
-- Impact			: 
-- Required grants  : 
-- Called by        : Online Dairy manula import
-- Sample Execution :
	Exec OnlineDiaryManualImport 'panel_export_20141214180000.csv',4,100,1,'TW','F:\fromftp\MalePanel\panel_export_20141214180000.csv','panel_export_20141214180000.csv',NEWID(),@Getdate
##########################################################################
...
Updates: 
	...
	-- 16/11/2015 LP incentives considered with CountryCode column  and GiftPrice =0
...
BEGIN TRY
	...
	DECLARE @CountryGUID AS NVARCHAR(100)
	DECLARE @PanelId AS NVARCHAR(100)
	...
	BEGIN TRY
		...
		(
				...
				LEFT JOIN TWN_OnLineDiaryProcessing.dbo.ftpFileImport f ON d.NPAN = f.[uid]
			...
			BEGIN TRY
				...
				SET @PeriodValue = (
						...
						WHERE CalendarId = @CalendarID
							...
							AND DATEADD(week, 0, CONVERT(DATE, @FileDate, 113)) BETWEEN CONVERT(DATE, StartDate, 113)
						...
						BEGIN
							...
							VALUES (
								...
								begin 
											...
											select @IncentiveAccountTransactionInfoId, 100, @GPSUser GPSUser, @Getdate GPSUpdateTimestamp ,  
											@Getdate CreationTimestamp , 0 as GiftPrice,
											'TransactionInfo' as Discriminator, @PointGUID  Point_Id, NULL RewardDeliveryType_Id, @CountryGUID
											...
											PackageId, TransactionInfo_Id, TransactionSource_Id, Depositor_Id, Panel_Id, 
											DeliveryAddress_Id, Account_Id, [Type],Country_Id, BatchId, TransactionId)
				...
				BEGIN
					...
					SET @ERROR_MESSAGE = @ERROR_MESSAGE + ' NPAN:' + @NPAN + ', ReceivedDate:' + @ReceivedDate + ',  FileDate: ' + @FileDate + ', Week: ' + @DiaryDateWeek + ', Period: ' + @DiaryDatePeriod + ', Year: ' + @DiaryDateYear
	...
	UPDATE DiaryImportAudit
	SET FileRowCount = @Passed
		,ErrorDescription = ' '
	WHERE [Id] = @AuditID
