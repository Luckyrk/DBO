CREATE PROCEDURE [dbo].[Usp_Frozen_Paineis_Domicilios_BR_Delta]  
(  
...
BEGIN  
 ...
 DECLARE @ViewName VARCHAR(500)='Frozen_Paineis_Domicilios_BR',@CountryFromDate DATETIME ,@CountryCurrentDate DATETIME ,@ServerFromDate DATETIME  
 ...
 IF Object_Id('dbo.Frozen_Paineis_Domicilios_BR_Delta_Current') IS NOT NULL  
 BEGIN  
 DROP TABLE Frozen_Paineis_Domicilios_BR_Delta_Current  
 END  
  IF Object_Id('dbo.FrozenPaineisDomiciliosChanges') IS NOT NULL    
 BEGIN    
 DROP TABLE FrozenPaineisDomiciliosChanges    
 END
 CREATE TABLE [dbo].[Frozen_Paineis_Domicilios_BR_Delta_Current](  
   idpainel INT,  
   iddomicilio INT,  
...
FROM Frozen_Paineis_Domicilios_BR WHERE GPSUpdateTimeStamp>= @CountryFromDate AND GPSUpdateTimeStamp <= @CountryCurrentDate      
UNION     
SELECT idpainel,iddomicilio,0 AS isDeleted      
FROM [Frozen_Paineis_Domicilios_BR_DeletedRecords]  WHERE GPSUpdateTimeStamp>= @ServerFromDate AND GPSUpdateTimeStamp <= @ServerCurrentDate 
  
  
 INSERT INTO Frozen_Paineis_Domicilios_BR_Delta_Current (idpainel,iddomicilio,Data_Entrada,Data_Saida,Cause_Saida,Tipo_Envio,[Load_date],[Country_Load_date])  
 SELECT T1.idpainel,T1.iddomicilio,Data_Entrada,Data_Saida,Cause_Saida,Tipo_Envio,@ServerCurrentDate,@CountryCurrentDate  
 FROM Frozen_Paineis_Domicilios_BR T1  
 ...
 LEFT JOIN Frozen_Paineis_Domicilios_BR_Delta_Current T2 ON T2.iddomicilio=T1.iddomicilio AND T1.idpainel=T2.idpainel  
 WHERE T2.iddomicilio IS NULL 
 
  DELETE FROM B    
 FROM FrozenPaineisDomiciliosChanges D    
 JOIN Frozen_Paineis_Domicilios_BR_Delta_BackUp B ON D.iddomicilio=B.iddomicilio  AND D.idpainel=B.idpainel 
 WHERE D.isDeleted=1  
    
    
 MERGE INTO Frozen_Paineis_Domicilios_BR_Delta_Current AS tgt  
USING  Frozen_Paineis_Domicilios_BR_Delta_BackUp AS src  
...
INSERT INTO Frozen_Views_Delta_Latam  
 ...
 FROM Frozen_Paineis_Domicilios_BR_Delta_Current T1   
 WHERE isUpdated=1  
 UNION  
  SELECT iddomicilio,@ServerCurrentDate,NULL,@CountryCode,@ViewName     
 ...
 DELETE B FROM     
Frozen_Paineis_Domicilios_BR_Delta_BackUp B    
JOIN Frozen_Paineis_Domicilios_BR_Delta_Current C ON C.iddomicilio=B.iddomicilio  AND C.idpainel=B.idpainel    
WHERE C.isUpdated=1    
      
  INSERT INTO Frozen_Paineis_Domicilios_BR_Delta_BackUp    
  SELECT * FROM Frozen_Paineis_Domicilios_BR_Delta_Current    
  WHERE isUpdated=1    
