CREATE PROCEDURE [dbo].[Usp_Frozen_HouseHold_GT_Delta]          
(          
...
AS          
BEGIN          
 DECLARE @ViewName VARCHAR(500)='Frozen_HouseHold_GT',@CountryFromDate DATETIME ,@CountryCurrentDate DATETIME ,@ServerFromDate DATETIME          
 ...
 IF Object_Id('dbo.Frozen_HouseHold_GT_Delta_Current') IS NOT NULL          
 BEGIN          
 DROP TABLE Frozen_HouseHold_GT_Delta_Current          
 END       
 ...
 CREATE TABLE [dbo].[Frozen_HouseHold_GT_Delta_Current](          
...
FROM [Frozen_HouseHold_GT_DeletedRecords]  WHERE  GPSUpdateTimeStamp>= @ServerFromDate AND GPSUpdateTimeStamp <= @ServerCurrentDate        
        
        
 INSERT INTO Frozen_HouseHold_GT_Delta_Current ([GroupBusinessId],[RegionGroup],[City code],[Zone Code],          
 ...
 LEFT JOIN Frozen_HouseHold_GT_Delta_Current T2 ON T2.GroupBusinessId=T1.GroupId        
 WHERE T2.GroupBusinessId IS NULL        
         
 DELETE FROM B          
 FROM FrozenHouseHoldChanges D          
 JOIN Frozen_HouseHold_GT_Delta_BackUp B ON B.[GroupBusinessId]=D.GroupId           
 WHERE D.isDeleted=1          
         
          
MERGE INTO Frozen_HouseHold_GT_Delta_Current AS tgt          
USING  Frozen_HouseHold_GT_Delta_BackUp AS src          
...
INSERT INTO Frozen_Views_Delta_Latam          
 ...
 FROM Frozen_HouseHold_GT_Delta_Current T1           
 WHERE isUpdated=1          
 UNION        
...
DELETE B FROM             
Frozen_HouseHold_GT_Delta_Backup B            
JOIN Frozen_HouseHold_GT_Delta_Current C ON C.[GroupBusinessId]=B.[GroupBusinessId]            
WHERE C.isUpdated=1          
          
  INSERT INTO Frozen_HouseHold_GT_Delta_Backup            
  SELECT * FROM Frozen_HouseHold_GT_Delta_Current            
  WHERE isUpdated=1           
