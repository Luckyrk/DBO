CREATE PROCEDURE [dbo].[Usp_Frozen_Demographic_GT_Delta]      
(      
...
BEGIN      
 ...
 DECLARE @ViewName VARCHAR(500)='Frozen_Demographic_GT',@CountryFromDate DATETIME ,@CountryCurrentDate DATETIME ,@ServerFromDate DATETIME      
 ...
 IF Object_Id('dbo.Frozen_Demographic_GT_Delta_Current') IS NOT NULL      
 BEGIN      
 DROP TABLE Frozen_Demographic_GT_Delta_Current      
 END   
 IF Object_Id('dbo.FrozenDemographicChanges') IS NOT NULL      
 BEGIN      
 DROP TABLE FrozenDemographicChanges      
 END      
 CREATE TABLE [dbo].[Frozen_Demographic_GT_Delta_Current](      
...
FROM Frozen_Demographic_GT WHERE GPSUpdateTimeStamp>= @CountryFromDate AND GPSUpdateTimeStamp <= @CountryCurrentDate          
UNION           
SELECT idDomicilio,0 AS isDeleted             
FROM [Frozen_Demographic_GT_DeletedRecords]  WHERE  GPSUpdateTimeStamp>= @ServerFromDate AND GPSUpdateTimeStamp <= @ServerCurrentDate        
        
      
 INSERT INTO Frozen_Demographic_GT_Delta_Current (idDomicilio,idPosse_Bem,data,Quantidade,[Load_date],[Country_Load_date])        
 SELECT T1.idDomicilio,T1.idPosse_Bem,T1.data,Quantidade,@ServerCurrentDate,@CountryCurrentDate         
 FROM Frozen_Demographic_GT T1        
 JOIN FrozenDemographicChanges T2 ON T1.idDomicilio=T2.idDomicilio      
       
 UPDATE T1 SET T1.isDeleted=1        
 FROM         
 FrozenDemographicChanges T1        
 LEFT JOIN Frozen_Demographic_GT_Delta_Current T2 ON T2.idDomicilio=T1.idDomicilio        
 WHERE T2.idDomicilio IS NULL        
       
  DELETE FROM B          
 FROM FrozenDemographicChanges D          
 JOIN Frozen_Demographic_GT_Delta_BackUp B ON B.idDomicilio=D.idDomicilio           
 WHERE D.isDeleted=1        
       
       
 MERGE INTO Frozen_Demographic_GT_Delta_Current AS tgt        
USING  Frozen_Demographic_GT_Delta_BackUp AS src        
...
INSERT INTO Frozen_Views_Delta_Latam        
 ...
 FROM Frozen_Demographic_GT_Delta_Current T1         
 WHERE isUpdated=1         
  UNION        
  SELECT idDomicilio,@ServerCurrentDate,NULL,@CountryCode,@ViewName           
...
DELETE B FROM         
Frozen_Demographic_GT_Delta_BackUp B        
JOIN Frozen_Demographic_GT_Delta_Current C ON C.idDomicilio=B.idDomicilio        
WHERE C.isUpdated=1        
          
  INSERT INTO Frozen_Demographic_GT_Delta_BackUp        
  SELECT * FROM Frozen_Demographic_GT_Delta_Current        
  WHERE isUpdated=1        
...
DROP TABLE FrozenDemographicChanges;        
 ...
 --EXEC SP_Rename 'dbo.Frozen_Demographic_GT_Delta_Current','Frozen_Demographic_GT_Delta_BackUp'      
