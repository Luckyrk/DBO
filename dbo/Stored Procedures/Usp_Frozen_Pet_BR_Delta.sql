CREATE Procedure [dbo].[Usp_Frozen_Pet_BR_Delta]      
(      
...
AS      
BEGIN      
 DECLARE @ViewName VARCHAR(500)='Frozen_Pet_BR',@CountryFromDate DATETIME ,@CountryCurrentDate DATETIME ,@ServerFromDate DATETIME      
 ...
 IF Object_Id('dbo.Frozen_Pet_BR_Delta_Current') IS NOT NULL          
 BEGIN          
 DROP TABLE Frozen_Pet_BR_Delta_Current          
 END       
 ...
 CREATE TABLE [dbo].[Frozen_Pet_BR_Delta_Current](      
  GroupBusinessId INT,      
...
FROM [Frozen_Pet_BR_DeletedRecords] 
WHERE  GPSUpdateTimeStamp>= @ServerFromDate AND GPSUpdateTimeStamp <= @ServerCurrentDate      
    
    
      
 INSERT INTO Frozen_Pet_BR_Delta_Current (GroupBusinessId,idanimal,idtamanho,idtipo_alimentacao,edad,PetNO,[Load_date],[Country_Load_date])      
 SELECT GroupBusinessId,idanimal,idtamanho,idtipo_alimentacao,edad,T1.PetNO,@ServerCurrentDate,@CountryCurrentDate      
 FROM Frozen_Pet_BR T1      
 JOIN FrozenPetChanges T2 ON T1.GroupBusinessId=T2.GroupId  AND T1.PetNO=T2.PetNo  
     
 UPDATE T1 SET T1.isDeleted=1      
 FROM       
 FrozenPetChanges T1      
 LEFT JOIN Frozen_Pet_BR_Delta_Current T2 ON T2.GroupBusinessId=T1.GroupId AND T1.PetNo=T2.PetNO      
 WHERE T2.GroupBusinessId IS NULL      
       
 DELETE FROM B        
 FROM FrozenPetChanges D        
 JOIN Frozen_Pet_BR_Delta_BackUp B ON B.[GroupBusinessId]=D.GroupId AND B.PetNo=D.PetNO        
 WHERE D.isDeleted=1     
      
 MERGE INTO Frozen_Pet_BR_Delta_Current AS tgt      
USING  Frozen_Pet_BR_Delta_BackUp AS src      
...
INSERT INTO Frozen_Views_Delta_Latam      
 ...
 FROM Frozen_Pet_BR_Delta_Current T1       
 WHERE isUpdated=1       
  UNION      
 ...
 DELETE B FROM         
Frozen_Pet_BR_Delta_BackUp B        
JOIN Frozen_Pet_BR_Delta_Current C ON C.GroupBusinessId=B.GroupBusinessId AND C.PetNO=B.PetNO        
WHERE C.isUpdated=1        
          
  INSERT INTO Frozen_Pet_BR_Delta_BackUp        
  SELECT * FROM Frozen_Pet_BR_Delta_Current        
  WHERE isUpdated=1        
